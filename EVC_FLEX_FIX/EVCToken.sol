...
contract EVC is Ownable, ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    function claimRewardFix(uint256 id, address user) public {
        infoFix storage stakedInfo = userStakedFix[user][id];
        ...
        require(stakedInfo.lastClaim < block.timestamp, "Cannot claim now, wait until claimable time");
        uint256 amount = getRewardFix(id, user);
        _transfer(address(this), user, amount);
        stakedInfo.lastClaim = block.timestamp;
    }
    ...
    //View
    function getRewardFix(uint256 id, address _user) public view returns(uint256) {
        infoFix storage stakedInfo = userStakedFix[_user][id];
        if (stakedInfo.stakeid != id) {
            return 0;
        }
        uint256 apy = getFixApy(stakedInfo.planid);
        uint256 perSecondReward = (fixPlans[stakedInfo.planid].perEVCPrice * apy) / (100 * 365 * 86400);
        uint256 stakeSeconds = block.timestamp - stakedInfo.lastClaim;
    ...
    function setFixStakePlan(uint256 id, uint256 _rewardBal, uint256 _maxApyPer, uint256 _perEVCPrice) external onlyOwner {
        //require(_rewardBal <= erc20Token.balanceOf(address(this)), "Given reward is less than balance");
    ...
    function claimRewardFlex(uint256 id) public {
        ...
        if (userStakedFlex[msg.sender][id].claimable > 0) {
            ...
            uint256 reward = getRewardFlex(id, msg.sender);
            _transfer(address(this), msg.sender, reward);
    ...
    //View
    function getRewardFlex(uint256 id, address _user) public view returns(uint256) {
